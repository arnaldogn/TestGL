...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    let container: Container = {
        let container = Container()
        container.register(FetchItemsServiceProtocol.self) { _ in FetchItemsService() }
        container.register(MainViewController.self) {
            return MainViewController(fetchService: $0.resolve(FetchItemsServiceProtocol.self)!)
        }
        return container
    }()
